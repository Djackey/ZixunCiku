...
# 这个curl方法是从zero那里扒过来的。http://www.seoqx.com/post/341
    ...
    def baidutzzq(self, cppagehtml):
        cpbdzqsj_data = []
        bdkzlj_data = []
        cplj_data = []

        bdsssoup = BeautifulSoup(cppagehtml, "lxml")
        bdsszqsjhtml = bdsssoup.select(".c-abstract span")
        for kuaizhaosj in bdsszqsjhtml:
            cpbdzqsj_data.append(kuaizhaosj.string)

        zchtml = soup.select(
            'a[data-click^="{\'rsv_snapshot\':\'1\']')
        for chanpinbt in zchtml:
            bdkzlj_data.append(chanpinbt['href'])

        bdsszqsjurlhtml = bdsssoup.select(".t a")
        for bdsszqsjurl in bdsszqsjurlhtml:
            res = requests.head(bdsszqsjurl['href'])
            cplj_data.append(res.headers['Location'])

        return cpbdzqsj_data

    def kuaizhaoxx(self, kuaizhaourl):
        kuaizhaohtml = curl(kuaizhaourl)
        kuaizhaosoup = BeautifulSoup(kuaizhaohtml, "lxml")
        kuaizhaosjhtml = kuaizhaosoup.select("div > #bd_snap_txt span")
        print kuaizhao
        kuaizhaosjre = re.findall(r"(.*)", kuaizhaosjhtml)
        for kuaizhaosj in kuaizhaosjre:
            print kuaizhaosj
