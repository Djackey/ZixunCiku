...
# 这个curl方法是从zero那里扒过来的。http://www.seoqx.com/post/341
    def curl(self, url, debug=False, **kwargs):
        while 1:
            try:
                s = StringIO.StringIO()
                c = pycurl.Curl()
    ...
    def d17chanpintz(self, cppagehtml):
        cpurltt_data = []
        cpbt_data = []
        cpyzlm_data = []
        cpfbrq_data = []
        chanpinsoup = BeautifulSoup(cppagehtml, "lxml")
        chanpintthtml = chanpinsoup.select(".company_name a")
        for chanpintt in chanpintthtml:
            cpurltt_data.append(chanpintt['title'])

        chanpinbthtml = chanpinsoup.select(
            ".productdetail_main_item h1 a")
        for chanpinbt in chanpinbthtml:
            # print chanpinbt.string
            cpbt_data.append(chanpinbt.string)

        chanpinyzlmhtml = chanpinsoup.select(
            "[class~=position_nav_left]")
        for chanpinyzlm in chanpinyzlmhtml:
            # print chanpinyzlm.strong.find_previous("a").string
            cpyzlm_data.append(
                chanpinyzlm.strong.find_previous("a").string)

        chanpinfbrqsj = re.findall(
            r"</span>\s+(\d+-\d+-\d+)\s至", cppagehtml)
        for farqsj in chanpinfbrqsj:
            cpfbrq_data.append(farqsj)
        chanpintz_data=[cpurltt_data,cpbt_data,cpyzlm_data,cpfbrq_data]
        return chanpintz_data
