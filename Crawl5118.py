...
# 这个curl方法是从zero那里扒过来的。http://www.seoqx.com/post/341
    ...
    def index5118(self, guanjianmuci):
        keywordindex_data = []
        bdkzlj_data = []
        cplj_data = []
        url5118="http://www.5118.com/seo/words/"+urllib.quote(guanjianmuci)
        url5118html=curl(url5118)
        soup = BeautifulSoup(url5118html, "lxml")
        ...
        for keywordssnum in keywordssnumhtml:
            keywordssnum.append(keywordssnum.string)

        data5118_data = [keywordindexhtml, keywordssnumhtml]

        return data5118_data